...
- **实时帧率显示**：性能面板「显示实时帧率」开关（`CFG.showFps`）打开后，在 canvas 左上角用 `ctx.fillText` 叠加显示实际渲染 FPS（`_updateFps()` 每 500ms 统计一次实际绘制帧数，`_drawFps()` 绘制）。Canvas 本身没有帧率 API，只能这样用 rAF 时间戳自行测量。
