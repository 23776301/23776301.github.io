...
- 默认方案为 B，选择保存在 `localStorage.paletteV2`；`:root` 的 `--accent` 默认值即 B 主题红 `#ef4444`（含 `--accent-a18`/`--accent-a28` 半透明变体）。
...
- 弹窗用**黑白键力度图**（`renderPaletteEndpoints`）替代原四个目标按钮：上下两条**直角梯形**色带（与配色方案菜单同形）——左竖直边高为右竖直边的一半，直观表达「轻→重」；每带两端各有一个**方形色块**即四个端点（白键轻/重、黑键轻/重），点击方块选择目标，选中方块加白边高亮。
- 取色板顶部预置常用品牌色（`PALETTE_PRESETS`），每色下方标注名称：哔哩粉 `#fb7299`、网抑红 `#c20c0c`、小书红 `#ff2442`、Q音绿 `#31c27c`、酷安绿 `#11aa66`、钉钉蓝 `#0089ff`、美团黄 `#ffc300`；点按即把当前目标色设为该色并实时应用。
- 取色区左半为 **2D 色板**（x=色相、y=明度，按当前饱和度渲染），右半为**色相/明度/饱和度三个 HSL 滑块**；2D 板圆点与三个滑块共四个手柄通过 `_syncEditHSL` / `_commitEditHSL` / `_updateSliderUI` 完全同步——拖动任一控件，其余三个与 2D 板同步跟随。
- **实时生效**：任何取色/预置色都经 `_applyCustomLive()` 立即重建 `PALETTES.custom`、`applyTheme()` 并持久化；因此弹窗**没有**叉号、取消、保存按钮，点遮罩空白处即可关闭。
- 取色板圆点滑块（`_positionBoardMarker`）按 `_editHSL` 的色相/明度定位，点预置色或手动取色都会同步移动。
...
| 默认主题色与按下态 | `:root` 默认 `--accent` 改为 B 主题红并在 `<head>` 内联脚本首屏应用已存配色，消除「先紫后红」闪烁；按钮悬停/按下由硬编码深紫改为主题色叠加半透明深色遮罩，速度按钮改用主题色变体 | `_pending_` |
