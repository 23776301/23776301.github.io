...
- **顶栏排列**：进度条单独置于顶栏最底部；上传 / 选谱后是**管理谱面**圆形图标按钮（`var(--accent)` 主题色，紧邻上传 MIDI 右侧）；播放控制、配色方案、颜色力度指示、全屏按钮排在进度条上方。
- **控制行（`.control-row`）**：播放 / 重播 / 循环均为**圆形图标按钮**（`.ctl-btn`，无文字）；顺序播放图标为**两个平行向右的箭头**。它们位于可收起区域之外，收起配色栏时仍可操作播放。
- **全屏按钮**：控制行**最右侧是配色栏展开/收起按钮**，其左侧是**全屏**按钮；全屏按钮**无背景、方形**，使用「带框箭头」矢量 SVG，对 `.visual-panel` 调用 `requestFullscreen()`，仅放大绘制区（Esc 退出）；进入 / 退出时图标切换为向内箭头。
