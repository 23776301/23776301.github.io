...
本文档按**主题分章**，并遵循「由浅入深」的阅读顺序：开篇是高度凝练的**卷首漫笔**，其后是较为易懂的**性能、配色、图标与视觉**，再往后才是架构、Canvas/UI、部署、调试、测试等技术深度内容。各章相互独立，可按需查阅。
...
音乐是时间的艺术，可视化让时间变得可见。把 MIDI 播出来不难，难的是让声音、光影与交互在浏览器里自然流动，并且足够轻、足够稳。

做得多了会发现，克制比堆砌更难：复杂留给实现，简单留给使用；能少一点就少一点，能稳一点就稳一点。
