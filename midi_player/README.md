...
| 内置谱表刷新与废弃归类 | `_getBuiltinList` 改联网刷新（`fetchFresh` no-store）+ 旧缓存回退，老用户可拿到最新内置谱表；对比新旧列表，把「之前存在、后来废弃」且用户缓存过的内置谱复制为「我的上传」用户谱（`_migrateDeprecatedBuiltins`），不主动删除任何用户缓存 | `_pending_` |
