...
本文档按**主题分章**，并遵循「由浅入深」的阅读顺序：开篇是高度凝练的**寄语**，其后是较为易懂的**性能、配色、图标与视觉**，再往后才是架构、Canvas/UI、部署、调试、测试等技术深度内容。各章相互独立，可按需查阅。
...
音乐是时间的艺术，可视化则让时间变得可见。这个播放器想做的，不只是把 MIDI 播出来，而是让声音、光影与交互在浏览器里自然流动：轻量、流畅、可靠，并在任何设备上都保持体面。

我们相信，好的工具是一种克制——不炫技、不堆砌，把复杂留给实现，把简单交给使用者。愿你在琴键落下、音符流淌之间，感受到计算机与音乐相遇时，那份朴素而令人惊叹的美。
