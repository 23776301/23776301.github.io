...
- 移动端菜单按钮位于可拖拽 FAB 组（组内仅此一个按钮），44px 圆形，默认在屏幕左侧中部；点击调用 `toggleMenu()` 展开/收起菜单面板。
- 拖动阈值 3px 用于区分「点击」与「拖动」；松手后吸附到屏幕左/右侧；位置持久化到 `localStorage.menuBtnPos`；纵向限制在视口 **10%–90%**，`resize`/`orientationchange`/`visualViewport` 变化时校正。
- 空闲 2s 淡出（透明度 0.5），任意交互时恢复。
- **PC 端不使用 FAB**（`.fab-group` 隐藏）：菜单按钮固定在上传 MIDI 按钮左侧（见 9.6）。

## 9.2 菜单面板（移动端：渐隐渐显）

- `.control-panel` 在 `max-width:800px` 下为**全宽、上方 2/3 高**（`height:66.67vh`）的浮层，用 `opacity` + `visibility` **渐隐渐显**展开/收起（不再有左右抽屉滑动动画）。
- `.overlay-mask` 半透明遮罩，点击调用 `toggleMenu()` 关闭；打开菜单时隐藏 FAB 组。
- 面板采用**半透明 + 背景模糊**风格，面板内的「透明 / 模糊」滑块可实时调节（见 9.7）。
...
- **菜单按钮**：PC 端菜单按钮固定在上传 MIDI 左侧；点击在「展开 / 折叠」左侧控制面板间切换；折叠时 `.layout.menu-collapsed` 把网格列改为 `0 1fr` 并隐藏面板，绘制区占满整行。
...
菜单面板（`.control-panel`）是**播放设置、音色与调试**的统一容器，固定尺寸、内容可滚动：

- **半透明 + 模糊**：面板背景为 `rgba(0,0,0,alpha)` + `backdrop-filter: blur()`，由面板内「透明 / 模糊」滑块调节并持久化（`panelOpacity` / `panelBlur`）。
...
- **降级自动展开**：勾选时，性能降级会（移动端）展开菜单面板并滚到日志底部；恢复时不自动收起。
...
| 主题 | 摘要 | 代表 commit |
| --- | --- | --- |
| PC 布局 | 整页锁定视口高度，绘制区自适应剩余高度，钢琴键始终贴底、无需滚动；配色栏压缩为单行 | `c1f7e49` |
| 菜单 | PC 端显示菜单按钮，点击折叠 / 展开左侧控制面板；FAB 在 PC 端取消左右吸附 | `c1f7e49` |
| 全屏 | 配色栏右侧新增全屏按钮，仅放大绘制区 | `c1f7e49` |

## 2026-09 菜单面板与调试整合

| 主题 | 摘要 | 代表 commit |
| --- | --- | --- |
| 调试整合 | debug 面板并入菜单面板「音色」下方，固定尺寸可滚动，新增「下载日志」；面板统一为半透明 + 模糊风格，可调透明度 / 模糊度 | `本提交` |
| 调试开关 | 菜单面板新增调试总开关（默认开启），关闭后无任何调试采集 / 监控 / 自动展开 | `本提交` |
| 移动端 | 菜单面板改为全宽、上方 2/3 高，渐隐渐显（去掉左右抽屉动画）；配色按钮恢复 2×2 | `本提交` |
| PC 顶栏 | 菜单按钮固定到上传 MIDI 左侧；进度条单列置底，其余控制移至其上方；全屏按钮改为方形双箭头图标 | `本提交` |
