...
- 前台 `playLoop` 中单帧触发上限 `MAX_TRIGGER_PER_FRAME = 256`。
- 后台由 `scheduleBg()` 前瞻调度：`SCHED_LOOKAHEAD = 2s`、`SCHED_INTERVAL = 250ms`；`playNote(..., when)` 支持指定音频时钟起始时间，供后台排程使用。
...
- **前台/后台双模式调度**：前台用 rAF 逐音符触发并渲染；切到后台时停掉渲染（后台无需下落动画、也不会切音色），改用 `setInterval` 前瞻调度器，提前 `SCHED_LOOKAHEAD = 2s` 按音频时钟把音符排入音频线程，从而后台持续出声；切回前台时立刻停后台调度、恢复音频上下文、**立即重绘一帧**并重启 rAF，同时重置帧跳变检测避免误判。移动端若系统挂起音频上下文，后台仍可能无声。
...
- 切后台持续播放 / 切回前台立即恢复渲染；移动端抽屉、FAB 拖动与吸附、弹窗、Toast。
...
- 后台标签页定时器会被节流（Chrome 约 ≥1s），后台调度器以 2s 窗口覆盖；但移动端操作系统可能在后台挂起音频上下文，此时后台仍无声，切回前台自动恢复。
...
| `MAX_TRIGGER_PER_FRAME` | 256 | 前台单帧最大触发音符数 |
| `SCHED_LOOKAHEAD` | 2s | 后台前瞻排程窗口 |
| `SCHED_INTERVAL` | 250ms | 后台调度轮询间隔 |
