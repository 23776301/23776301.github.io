...
| 变更 | 说明 | 提交 |
| --- | --- | --- |
| 73 键音域修正 | 73 键由越界 E2–E8（40–112）改为标准 E1–E7（28–100），缩放 1.209×、偏移 44.49% 正常（原偏移被钳到 100%） | `_pending_` |
| 移动端按钮网格 | 第一行与第二行统一网格：左右间距 = 上下间距 = 6px，10 个圆形按钮 `aspect-ratio:1` 等比缩放（修复椭圆拉伸），第二行 8 个恰好填满整宽且第一行最左/最右与第二行对齐 | `_pending_` |
| 门户描述 | 主页 MIDI 播放器卡片描述新增「多种键型钢琴 / 音游模式 / CDN竞速」 | `_pending_` |
