...
- **顶栏两行**：第一行 `重播 / 选谱列表 / 全屏`（重播在左、全屏在右）；第二行 `播放·暂停 / 下一首 / 列表·单曲循环 / 谱面管理 / 上传谱面 / 设置 / 配色 / 调试`。进度条与统计信息仍单独置底。
...
- **按钮**：A/B/C/自定义四个 `.pal-btn` 为 32px 圆形，与播放控制按钮同尺寸、单行对齐；**始终为不透明主题紫底白字**，选中项外圈加 `box-shadow:0 0 0 2px #fff` 白框标识（不再用半透明背景）。已删除「配色方案」竖排文字标签。
